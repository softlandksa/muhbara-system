...
function getMissingEnvVars(): string[] {
  ...
  // Credentials: GOOGLE_SERVICE_ACCOUNT_JSON is preferred; fallback requires both separate vars
  ...
  if (!process.env.GOOGLE_SHEETS_RANGE)          missing.push("GOOGLE_SHEETS_RANGE");
  if (!process.env.GOOGLE_SHEETS_SYNC_SECRET)    missing.push("GOOGLE_SHEETS_SYNC_SECRET");
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (
      ...
      message.includes("MISSING_CLIENT_EMAIL") ||
      ...
      userMsg    = "تنسيق مفتاح الخدمة غير صحيح — تأكد من صحة private_key داخل GOOGLE_SERVICE_ACCOUNT_JSON";
      httpStatus = 503;
    } else if (
